...
Triggering in regions independently.
...
```python
df_seas5_season.set_index("pcode").loc["BF49"].set_index("month").loc[
    7
].sort_values("mean_rp")
```
...
```python
(total_years + 1) / len(set(triggered_years))
```
...
```python
df_plot["trig"] = df_plot["trig"].astype(bool)
```
...
```python
df_plot[df_plot["trig"]]["year"].nunique()
```

```python
for name, group in df_plot[df_plot["trig"]].groupby("name"):
    display(group)
    print(group["year"].nunique())
```

```python
df_plot[df_plot["year"] == 2006]
```

```python
df_plot["year"].nunique()
```
